...
Move right forward and add characters.
...
Store the current window if it is the shortest.
Move left forward to remove characters and make the window smaller.
If the window remains valid, continue moving left.

If removing a character makes it invalid:

have--;

The while loop stops.

The outer loop moves right forward again and searches for the missing character.

The previously valid window is already saved using:

bestStart
bestLength

So it is safe for the current window to become invalid.*/